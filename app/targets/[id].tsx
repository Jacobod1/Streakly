...
export default function EditTargetScreen() {
  ...
  useEffect(() => {
    if (!id || !user) return;
    (async () => {
      const [[target], hs] = await Promise.all([
        db.select().from(targets).where(eq(targets.id, id)),
        db.select().from(habits).where(eq(habits.user_id, user.id)),
      ]);
      if (target) {
        setHabitId(target.habit_id);
  ...
  async function handleSave() {
    ...
    setSaving(true);
    try {
      await db.update(targets).set({ habit_id: habitId, period, target_value: val }).where(eq(targets.id, id));
      router.back();
    } catch (e: any) {
  ...
  return (
    <KeyboardAvoidingView style={s.root} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
      <ScrollView contentContainerStyle={s.content} keyboardShouldPersistTaps="handled">
        ...
        <Text style={s.hint}>{period === 'weekly' ? 'per week' : 'per month'}</Text>
